...
def plot_topk_curve(ensemble_df: pd.DataFrame, outpath: Path) -> None:
    ...
    plt.ylabel("2-fold CV Spearman correlation")
...
def plot_figure3_style_scatter(
    ...
    plt.ylabel("Actual")
    plt.title(f"DW-NOMINATE vs Top-{K} ensemble prediction")
    plt.tight_layout()
    plt.savefig(outpath, dpi=300, bbox_inches="tight")
    plt.close()
...
def plot_heatmap(corr: np.ndarray, outpath: Path) -> None:
    ...
    plt.colorbar(im, label="2-fold CV Spearman correlation")
    plt.xlabel("Head")
    plt.ylabel("Layer")
...
def main():
    ...
    if 32 in ensemble_preds_dict:
        print("Saved Figure-3-style scatter to:", outdir / "figure3_style_top32_scatter.png")
