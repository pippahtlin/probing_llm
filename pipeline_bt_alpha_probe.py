...
4) training ridge linear probes for BT score,
...
Usage example:
python mistral_bt_pipeline.py \
    --bt-csv /home/pippalin2/probing_llm/data/gpt_bt_score.csv \
    --outdir ./mistral_bt_outputs
...
- Following the Bradley-Terry paper, the default target is bt_score.
"""
...
DEFAULT_ALPHA_GRID = (0.0, 0.001, 0.01, 0.1, 1.0, 100.0, 1000.0)
...
def infer_name_column(df: pd.DataFrame) -> str:
    ...
    if not obj_cols:
        raise ValueError("Could not infer senator-name column in BT CSV.")
    return max(obj_cols, key=lambda c: df[c].nunique())


def infer_bt_score_column(df: pd.DataFrame) -> str:
    # BT pipeline uses bt_score.
    exact_candidates = [
        "bt_score",
        "bt score",
        "bradley_terry_score",
    ...
    fallback_candidates = [
        ...
        if "bt" in c.lower() and "score" in c.lower()
    ]
    if fallback_candidates:
        return fallback_candidates[0]

    raise ValueError(
        "Could not find BT score column. "
        "Expected something like 'bt_score'."
    )


def load_bt_data(csv_path: Path, first_n: Optional[int] = None) -> pd.DataFrame:
    df = pd.read_csv(csv_path)

    name_col = infer_name_column(df)
    bt_score_col = infer_bt_score_column(df)

    out = df[[name_col, bt_score_col]].copy()
    out.columns = ["senator", "bt_score"]
    out["senator"] = out["senator"].map(normalize_name)
    out["bt_score"] = pd.to_numeric(out["bt_score"], errors="coerce")
    out = out.dropna(subset=["senator", "bt_score"]).drop_duplicates(subset=["senator"])
...
def tune_alpha_global(
    ...
    """Tune one global ridge alpha for the entire run, following the paper's setup.

    We evaluate each candidate alpha with the same 2-fold CV scheme used for probing,
    compute headwise CV Spearman correlations for all heads, and then choose the alpha
    with the highest mean correlation across heads. The selected alpha is then fixed for
    all heads/folds in the rest of the pipeline.
    """
    ...
    for alpha in alpha_grid:
        ...
        mean_corr = float(np.mean(corr_vals)) if corr_vals else float("-inf")
        median_corr = float(np.median(corr_vals)) if corr_vals else float("-inf")
        max_corr = float(np.max(corr_vals)) if corr_vals else float("-inf")
        rows.append(
            {
                "alpha": float(alpha),
                "mean_headwise_spearman": mean_corr,
                "median_headwise_spearman": median_corr,
                "max_headwise_spearman": max_corr,
            }
        )

    alpha_df = pd.DataFrame(rows).sort_values(
        ["mean_headwise_spearman", "median_headwise_spearman", "max_headwise_spearman", "alpha"],
...
def tune_alpha_inner_cv(
    ...
    if len(y_train) < 4:
        return 1.0 if 1.0 in alpha_grid else float(alpha_grid[0])

    inner_splits = min(2, len(y_train))
    if inner_splits < 2:
        return 1.0 if 1.0 in alpha_grid else float(alpha_grid[0])

    kf_inner = KFold(n_splits=inner_splits, shuffle=True, random_state=seed)
    best_alpha = None
    best_score = -np.inf

    for alpha in alpha_grid:
        inner_preds = np.full(len(y_train), fill_value=np.nan, dtype=np.float32)
        ...
        for inner_train_idx, inner_val_idx in kf_inner.split(X_train):
            ...
            inner_preds[inner_val_idx] = model.predict(X_inner_val)

        mask = ~np.isnan(inner_preds)
        if mask.sum() < 3:
            score = -np.inf
        else:
            score, _ = spearmanr(y_train[mask], inner_preds[mask])
            if np.isnan(score):
                score = -np.inf
...
def cv_spearman_for_head(
    ...
    for train_idx, test_idx in kf.split(X_head):
        ...
        alpha_to_use = float(alpha)
        if alpha_grid is not None:
            alpha_to_use = tune_alpha_inner_cv(
                X_train=X_train,
                y_train=y_train,
        ...
        json.dump(
            {
                ...
                "ridge_alpha_grid": list(alpha_grid) if alpha_grid is not None else None,
                "cv_splits": n_splits,
                "seed": seed,
            },
            f,
...
def fit_final_probe_for_head(
    X_head: np.ndarray,
    y: np.ndarray,
    alpha: float = 1.0,
) -> Dict[str, np.ndarray]:
    scaler = StandardScaler()
    X_scaled = scaler.fit_transform(X_head)
...
def save_probe_artifacts(
    ...
    outdir: Path,
    alpha: float = 1.0,
    top_heads_dict: Optional[Dict[int, List[Tuple[int, int]]]] = None,
) -> None:
    X, y = flatten_features(records)
    ...
    probe_artifacts = {
        ...
        "probes": {},
        "topk_selected_heads": top_heads_dict if top_heads_dict is not None else {},
    }

    for layer in range(NUM_LAYERS):
        for head in range(NUM_HEADS):
            X_head = X[:, layer, head, :]
            probe_info = fit_final_probe_for_head(
                X_head=X_head,
                y=y,
                alpha=alpha,
            )
            probe_artifacts["probes"][(layer + 1, head + 1)] = probe_info
        ...
        top_ranked.append(
            {
                "layer": int(row["layer"]),
                "head": int(row["head"]),
                "spearman_cv": float(row["spearman_cv"]),
            }
        )
    probe_artifacts["head_ranking"] = top_ranked

    torch.save(probe_artifacts, outdir / "probe_artifacts.pt")
...
def plot_figure3_style_scatter(
    ...
    plt.ylabel("Actual")
    plt.title(f"bt_score vs Top-{K} ensemble prediction")
    plt.tight_layout()
...
def plot_heatmap(corr: np.ndarray, outpath: Path) -> None:
    ...
    #plt.title("Mistral attention-head probe performance for bt_score")
    plt.tight_layout()
    plt.savefig(outpath, dpi=300, bbox_inches="tight")
    plt.close()
...
def main():
    ...
    parser.add_argument(
        "--bt-csv",
        type=str,
        default="/home/pippalin2/probing_llm/data/gpt_bt_score.csv",
        help="Path to BT CSV.",
    )
    parser.add_argument("--outdir", type=str, default="mistral_bt_outputs")
    parser.add_argument("--cache-dir", type=str, default=DEFAULT_CACHE_DIR)
    parser.add_argument(
        "--ridge-alpha",
        type=float,
        default=None,
        help="Optional fixed ridge alpha. If omitted, tune one global alpha once and then use it everywhere.",
    )
    parser.add_argument(
        "--alpha-grid",
        type=float,
        ...
        help="Candidate ridge alphas used for one-time global alpha selection.",
    )
    ...
    bt_df = load_bt_data(Path(args.bt_csv), first_n=args.limit)
    first5_df = save_first_five_prompts(bt_df, outdir)

    print(f"Loaded {len(bt_df)} lawmakers from {args.bt_csv}")
    print("Target column: bt_score")
    ...
    records = extract_records(bt_df, tokenizer, model, outdir)
    print("Saved activation records to:", outdir / "all_records.pt")

    if args.ridge_alpha is None:
        print(f"Tuning one global ridge alpha from grid {args.alpha_grid} using 2-fold CV, then fixing it for all heads...")
        selected_alpha, alpha_search_df = tune_alpha_global(
            records=records,
            alpha_grid=args.alpha_grid,
            n_splits=args.cv_splits,
            seed=args.seed,
        )
        alpha_search_df.to_csv(outdir / "global_alpha_search.csv", index=False)
        print(f"Selected global ridge alpha: {selected_alpha}")
        print("Saved alpha search results to:", outdir / "global_alpha_search.csv")
        ridge_alpha = selected_alpha
        alpha_grid = None
    else:
        ridge_alpha = float(args.ridge_alpha)
        alpha_grid = None
        print(f"Using user-specified fixed ridge alpha: {ridge_alpha}")
    ...
    print(f"Ridge alpha used for all probes: {ridge_alpha}")
    ...
    print("Saved probe artifacts to:", outdir / "probe_artifacts.pt")
    print("Done.")
